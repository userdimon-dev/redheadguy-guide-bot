...
- 🌐 **Веб-редактор (FastAPI)** — полноценная панель управления через браузер:
  ...
  - 🔒 скрытие категорий и гайдов (черновики / soft delete `is_hidden`),
  - 🎛️ настройка порядка и сетки кнопок (`sort_order`, `row_number`),
...
- 🎛️ Настройка порядка (`sort_order`) и рядов кнопок (`row_number`) для сетки в боте и на сайте
- 🏷️ Бейджи статуса публикации и параметров сетки в админ-панели
- 🧪 Обновленные автотесты `pytest`
